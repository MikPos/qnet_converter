rule [
	left [
		node [ id 1 label "C" ]
		node [ id 2 label "O" ]
		edge [ source 2 target 1 label "-" ]
		node [ id 3 label "H" ]
		edge [ source 3 target 1 label "-" ]
		node [ id 4 label "H" ]
		edge [ source 4 target 1 label "-" ]
		node [ id 5 label "H" ]
		edge [ source 5 target 1 label "-" ]
		node [ id 6 label "O" ]
		edge [ source 6 target 2 label "-" ]
		node [ id 7 label "O" ]
		node [ id 8 label "C" ]
		edge [ source 8 target 14 label "-" ]
		edge [ source 8 target 7 label "=" ]
		node [ id 9 label "C" ]
		edge [ source 9 target 10 label "=" ]
		edge [ source 9 target 15 label "-" ]
		edge [ source 9 target 8 label "-" ]
		node [ id 10 label "C" ]
		node [ id 11 label "C" ]
		edge [ source 11 target 10 label "-" ]
		node [ id 12 label "C" ]
		edge [ source 12 target 11 label "-" ]
		node [ id 13 label "C" ]
		edge [ source 13 target 12 label "-" ]
		node [ id 14 label "O" ]
		edge [ source 14 target 11 label "-" ]
		node [ id 15 label "O" ]
		node [ id 16 label "O" ]
		edge [ source 16 target 13 label "-" ]
		node [ id 17 label "O" ]
		edge [ source 17 target 12 label "-" ]
		node [ id 18 label "H" ]
		edge [ source 18 target 15 label "-" ]
		node [ id 19 label "O" ]
		edge [ source 19 target 10 label "-" ]
		node [ id 20 label "H" ]
		edge [ source 20 target 19 label "-" ]
		node [ id 21 label "H" ]
		edge [ source 21 target 17 label "-" ]
		node [ id 22 label "H" ]
		edge [ source 22 target 16 label "-" ]
		node [ id 23 label "H" ]
		edge [ source 23 target 11 label "-" ]
		node [ id 24 label "H" ]
		edge [ source 24 target 12 label "-" ]
		node [ id 25 label "H" ]
		edge [ source 25 target 13 label "-" ]
		node [ id 26 label "H" ]
		edge [ source 26 target 13 label "-" ]
	]
	right [
		node [ id 1 label "C" ]
		node [ id 2 label "O" ]
		edge [ source 2 target 1 label "-" ]
		node [ id 3 label "H" ]
		edge [ source 3 target 1 label "-" ]
		node [ id 4 label "H" ]
		edge [ source 4 target 1 label "-" ]
		node [ id 5 label "H" ]
		edge [ source 5 target 1 label "-" ]
		node [ id 6 label "O" ]
		edge [ source 6 target 10 label "-" ]
		edge [ source 6 target 2 label "-" ]
		node [ id 7 label "O" ]
		node [ id 8 label "C" ]
		edge [ source 8 target 14 label "-" ]
		edge [ source 8 target 7 label "=" ]
		node [ id 9 label "C" ]
		edge [ source 9 target 10 label "-" ]
		edge [ source 9 target 15 label "-" ]
		edge [ source 9 target 8 label "-" ]
		node [ id 10 label "C" ]
		node [ id 11 label "C" ]
		edge [ source 11 target 10 label "-" ]
		node [ id 12 label "C" ]
		edge [ source 12 target 11 label "-" ]
		node [ id 13 label "C" ]
		edge [ source 13 target 12 label "-" ]
		node [ id 14 label "O" ]
		edge [ source 14 target 11 label "-" ]
		node [ id 15 label "O" ]
		node [ id 16 label "O" ]
		edge [ source 16 target 13 label "-" ]
		node [ id 17 label "O" ]
		edge [ source 17 target 12 label "-" ]
		node [ id 18 label "H" ]
		edge [ source 18 target 15 label "-" ]
		node [ id 19 label "O" ]
		edge [ source 19 target 10 label "-" ]
		node [ id 20 label "H" ]
		edge [ source 20 target 19 label "-" ]
		node [ id 21 label "H" ]
		edge [ source 21 target 17 label "-" ]
		node [ id 22 label "H" ]
		edge [ source 22 target 16 label "-" ]
		node [ id 23 label "H" ]
		edge [ source 23 target 11 label "-" ]
		node [ id 24 label "H" ]
		edge [ source 24 target 12 label "-" ]
		node [ id 25 label "H" ]
		edge [ source 25 target 13 label "-" ]
		node [ id 26 label "H" ]
		edge [ source 26 target 13 label "-" ]
	]
]
